...
use coidnggita
```
- It will make a new database named `codinggita`.

```js
db.createCollection("students");
```
- It will make a new collection named `students`.

```js
db.createCollection("courses");
```
- It will also make a new collection named `courses`.
...
```
- It will add the data in collection named `students`.
...
```
- It will add the data in the collection named `courses`.
  ...
  }]);
{
  acknowledged: true,
  insertedIds: {
    '0': ObjectId('677bfd9f7b990b8dad8167cb'),
    '1': ObjectId('677bfd9f7b990b8dad8167cc')
  }
}
db.courses.insertMany([
  { 
    ...
    "courseName": "Introduction to Programming", 
    "credits": 3, 
    "instructor": "Prof. Dangar" 
  },
  { 
    "courseCode": "CS102", 
    "courseName": "Data Structures", 
    "credits": 3, 
    "instructor": "Prof. Barad" 
  }]);
{
  acknowledged: true,
  insertedIds: {
    '0': ObjectId('677bfdd47b990b8dad8167cd'),
    '1': ObjectId('677bfdd47b990b8dad8167ce')
  }
}
db.courses.updateOne({courseName:"Data Structures"},{$set:{courseName:"Algoritham"}});
{
  acknowledged: true,
  insertedId: null,
  matchedCount: 1,
